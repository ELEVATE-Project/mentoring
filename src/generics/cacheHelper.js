...
//add validation error for fetching keys
...
async function evictOrgByPattern(tenantCode, orgCode, { patternSuffix = '*' } = {}) {
	if (!tenantCode || !orgCode) return
	const pattern = `tenant:${tenantCode}:org:${orgCode}:${patternSuffix}`
	await scanAndDelete(pattern)
}
